...
In this case, actually the sub-query returns the outcome from different tables, we can use join to get the same result.
...
We don't use distinct in count this time, because order_num is the primary key in orders
		...
		SELECT c.cust_id, 
			(SELECT COUNT(*)
    			FROM orders o
    		WHERE o.cust_id = c.cust_id) sum_order
		FROM customers c;
